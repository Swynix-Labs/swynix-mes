...
frappe.ui.form.on("Circle Packing Log", {
	onload(frm) {
		// Auto-generate packing_id for new documents
		if (frm.is_new() && !frm.doc.packing_id) {
			frappe.call({
				method: 'swynix_mes.swynix_mes.doctype.circle_packing_log.circle_packing_log.generate_packing_id',
				callback(r) {
					if (r.message) {
						frm.set_value('packing_id', r.message);
					}
				}
			});
		}
